...
const basePath = path.join(__dirname, '../../tower_defense_client/assets');
...
export const loadGameAssets = async () => {
  try {
    const [stages, monsters, monsterUnlocks, init_datas] = await Promise.all([
      readFileAsync('stage.json'),
      readFileAsync('monster.json'),
      readFileAsync('monster_unlock.json'),
      readFileAsync('init.json'),
    ]);
    gameAssets = { stages, monsters, monsterUnlocks, init_datas };
    return gameAssets;
  } catch (error) {
